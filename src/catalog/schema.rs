...
struct Inner {
    ...
    tables: BTreeMap<TableIdT, Arc<Table>>,
    next_table_id: TableIdT,
}

impl Schema {
    pub(crate) fn new(schema_id: SchemaIdT, schema_name: String) -> Schema {
        Schema {
            schema_id,
    ...
    pub(crate) fn add_table(
        ...
        let table_catalog = Arc::new(Table::new(
            table_id,
            table_name.clone(),
            columns,
    ...
    pub(crate) fn get_all_tables(&self) -> BTreeMap<TableIdT, Arc<Table>> {
    ...
    pub(crate) fn get_table_by_id(&self, table_id: TableIdT) -> Option<Arc<Table>> {
        let inner = self.inner.lock();
        inner.tables.get(&table_id).cloned()
    }

    pub(crate) fn get_table_by_name(&self, name: &str) -> Option<Arc<Table>> {
        let inner = self.inner.lock();
        inner
            .table_idxs
            .get(name)
...
mod tests {
    ...
    fn test_schema_catalog() {
        ...
        let mut schema_catalog = Schema::new(0, "test_scheme".to_string());
