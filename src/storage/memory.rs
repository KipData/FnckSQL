...
impl MemStorage {
    pub fn new() -> MemStorage {
        Self {
            inner: Arc::new(Cell::new(StorageInner {
...
impl Storage for MemStorage {
    ...
    async fn table(&self, name: &String) -> Option<&TableCatalog> {
        unsafe { self.inner.as_ptr().as_ref().unwrap().root.get_table(name) }
    }
...
impl Transaction for MemTable {
    ...
    fn append(&mut self, tuple: Tuple, is_overwrite: bool) -> Result<(), StorageError> {
        let tuples = unsafe { self.tuples.as_ptr().as_mut() }.unwrap();
